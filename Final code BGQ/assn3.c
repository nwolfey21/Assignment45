...
double clock_rate=1600000000.0;//3200000000.0;//2666700000.0; // change to 1600000000.0 for Blue Gene/Q
...
{ 
    ...
    long mythrd;  
    int numthrds, dataperthread, datapernode, start, end;
    double** A, ** B, ** C;
...
int main(int argc, char** argv)
{
	...
	double **tB=NULL;
	/***Other Variables***/
	int i, j, datapernode, flag, tasks, nodes, ranksPerFile=0;
	int commSize, myRank, send, recv, count, blocking=0;
	long long start_time, time_cycles, sum_cycles;
	long long start_compute, compute_cycles, min_cycles, max_cycles;
	long long start_parallelIO, write_cycles, min_write_cycles, max_write_cycles, sum_write_cycles;
	long long sum_compute;
	long long max_compute;
	long long min_compute;
		...
		if(argv[i][0] == '-') 
		{
			switch(argv[i][1]) 
			{
				...
				case 'r':	ranksPerFile = atoi(argv[i+1]);	//1=all ranks use one file, 4=4ranks per file, etc
							break;
				case 'b':	blocking = atoi(argv[i+1]);		//1=write files to 8MB blocks, 0=write data compact
							break;
				case 'p':	pflag=1;						//just test parallel IO
							break;
				case 'a':	numthrds = atoi(argv[i+1]);
	...
	//-----Initialize Matrices------//
	A = createMatrix(datapernode, matrix_size);
	B = createMatrix(matrix_size, datapernode);
	C = createMatrix(datapernode, matrix_size);
	tB = createMatrix(matrix_size, datapernode);
	...
	for( i = 0; i < datapernode; i++ )  
	{
		for( j = 0; j < matrix_size; j++) 
		{
			...
			if(pflag)	
			{
				C[i][j] = genrand_res53(); // (double)i*j;
			}
		}
	}
    mdata.A = A;
	...
	if(!pflag)	
	{
		for(count=0; count<commSize; count++)
		{
			...
			if(count<commSize-1)
			{
				...
				MPI_Isend(&A[0][0], datapernode*matrix_size, MPI_DOUBLE, send, 1, MPI_COMM_WORLD, &srequest);
				//---Recieve Data in B Matrix---//
				MPI_Irecv(&tB[0][0], datapernode*matrix_size, MPI_DOUBLE, recv, MPI_ANY_TAG, MPI_COMM_WORLD, &rrequest);
				flag = 0;
...
//	MPI_Barrier(MPI_COMM_WORLD);
	...
	if(ranksPerFile !=0)	//All cases other than 1 file for all ranks
	{
		...
		err = MPI_File_open( MPI_COMM_SELF, filename, MPI_MODE_RDWR | MPI_MODE_CREATE | MPI_MODE_DELETE_ON_CLOSE, MPI_INFO_NULL, &fh );
		...
		if(blocking)
		{
			offset = (long long)(((long long)myRank%(long long)ranksPerFile)*(long long)BLOCK_SIZE);		//Move offset over 8MB (1block)
		}
		else
		{
			offset = (long long)(((long long)myRank%ranksPerFile)*(long long)datapernode*(long long)matrix_size*(long long)8);	//Set the offset into the file for first data location without blocking
		}
		MPI_File_write_at(fh, offset, &mdata.C[0][0], datapernode*matrix_size, MPI_DOUBLE, &status);	//Write vector C[i]
		/****Start Verify Data was written to file****/
		err = MPI_Get_elements(&status, MPI_BYTE, &count);
		if(err != MPI_SUCCESS)
			printf("Write Failed\n");
		if(count != datapernode*matrix_size*8)
		{
			printf("Rank%d wrote %d bytes instead of %d bytes\n",myRank,count,(int)(datapernode*matrix_size*sizeof(double)));
		}
		else 
		{
			if(myRank%128==0 &&i%32==0)
	...
	{
		sprintf(filename, "data/cMatrix.dat"); //bglockless:
		...
		if(blocking)
		{
			offset = (long long)((long long)myRank*(long long)BLOCK_SIZE);		//Move offset over 8MB (1block)
		}
		else
		{
			offset = (long long)((long long)myRank*(long long)datapernode*(long long)matrix_size*(long long)8);	//Set the offset into the file for first data location without blockin
		}
		MPI_File_write_at_all(fh, offset, &mdata.C[0][0], datapernode*matrix_size, MPI_DOUBLE, &status);	//Write vector C[i]
		/****Start Verify Data was written to file****/
		err = MPI_Get_elements(&status, MPI_BYTE, &count);
		if(err != MPI_SUCCESS)
			printf("Write Failed\n");
	...
	write_cycles = rdtsc() - start_parallelIO;
...
//	MPI_File_close( &fh );					//Close the MPI File
	...
	//  parallel IO write  //
	MPI_Allreduce(&write_cycles,&sum_write_cycles,1,MPI_LONG_LONG,MPI_SUM,MPI_COMM_WORLD);
	MPI_Allreduce(&write_cycles,&max_write_cycles,1,MPI_LONG_LONG,MPI_MAX,MPI_COMM_WORLD);
	MPI_Allreduce(&write_cycles,&min_write_cycles,1,MPI_LONG_LONG,MPI_MIN,MPI_COMM_WORLD);
	...
	if(myRank == 0)
	{
		...
		fprintf(dataFile,"%d %d %d %d %d ",nodes, tasks,ranksPerFile,blocking,numthrds);
		...
		fprintf(dataFile,"%lld %lf ",sum_compute,(double)sum_compute/clock_rate);
		//parallelIO write times
		fprintf(dataFile,"%lld %lf ",max_write_cycles,(double)max_write_cycles/clock_rate);
		fprintf(dataFile,"%lld %lf ",min_write_cycles,(double)min_write_cycles/clock_rate);
		fprintf(dataFile,"%lld %lf ",sum_write_cycles/commSize,(double)(sum_write_cycles/commSize)/clock_rate);
		fprintf(dataFile,"%lld %lf\n",sum_write_cycles,(double)sum_write_cycles/clock_rate);
	}
	MPI_Finalize();
