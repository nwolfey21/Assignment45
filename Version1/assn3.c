...
double *inB=NULL;
double **C=NULL;
unsigned int matrix_size=16384;
...
void matrix_multiply( int rank, int id, double **A, double **B, double **C, int size, int slice )
{
    int i=0, j=0, k=0;
...
//			printf("rank:%d C[%d][%d]:%f\n",rank, i+rank*slice,j+id*slice,C[i][j]);
		}
	}
//	printf("Rank:%d Completed A[%d][%d] in time %lf secs \n", rank, rank*slice, id*slice, ((double)current - (double)last)/clock_rate);
}

int main(int argc, char** argv)
{
	int i, j, n, index, datapernode, flag, tasks, nodes;
...
//	printf("rank\n");
	...
	sprintf(filePath, "data/data.dat", commSize, tasks);
...
//		outB[i] = (double *)calloc( datapernode, sizeof(double));
	...
	for( i = 0; i < datapernode; i++ ) 
	{
		C[i] = (double *)calloc( datapernode, sizeof(double));
	}
//	printf("rank:%d here3\n",myRank);

	if(myRank == 0)
	{
		printf("Matrix Size:%d datapernode:%d commSize:%d\n",matrix_size, datapernode, commSize);
	}
...
//						printf("rank:%d A[%d][%d]:%f B[%d][%d]:%f\n",myRank, i+myRank*datapernode,j+myRank*datapernode,A[i][j],j+myRank*datapernode,i+myRank*datapernode,B[j][i]);
		}
	}
	//--Start Timer--//
	start_time = rdtsc();
	...
	for(count=0; count<commSize; count++)
	{
		...
		matrix_multiply(myRank, id, A, B, C, matrix_size, datapernode);
	...
	//			printf("rank:%d received from rank:%d\n",myRank,recv);
			...
			for( i = 0; i < matrix_size; i++ )  /* Initialization */
			{
				for( j = 0; j < datapernode; j++ )
				{
					n = j + i*datapernode;
					B[i][j] = inB[n];
//					printf("rank:%d inB[%d]:%f\n",myRank,n,inB[n]);
//					printf("----rank:%d B[%d][%d]:%f\n",myRank, i+myRank*datapernode,j+myRank*datapernode,B[i][j]);
				}
			}
			copy_cycles += rdtsc() - start_copy;
		}
		if(myRank == 0 && count%(commSize/100) == 0)
		{
			printf("Count %d completed\n", count);
		}
	}
...
//	printf("Rank %d Transfer_Cycles:%lld in time %lf secs\n",myRank,transfer_cycles,(double)transfer_cycles/clock_rate);
//	printf("Rank %d Compute_Cycles:%lld in time %lf secs\n",myRank,compute_cycles,(double)compute_cycles/clock_rate);
//	printf("Rank %d Copy_Cycles:%lld in time %lf secs\n",myRank,copy_cycles,(double)copy_cycles/clock_rate);
